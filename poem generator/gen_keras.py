...
def test(sentence):
    ...
    for i in range(len(sentence)):
        if((sentence[i]=='，' or sentence[i]=='。' or sentence[i]==' ') and (i+1)%6!=0):
            print('invalid, re-generate, exp1')
            return False
        if ((i + 1) % 6 == 0 and (sentence[i] != '，' and sentence[i] != '。')):
            print('invalid, re-generate, exp2')
            return False
    return True
...
for i in range(3):
    ...
    print("藏头诗")
    ARG = '何何盛峰'
    sen = model.predict_hide(ARG,temperature=1)
    while (not test(sen)):
        sen = model.predict_hide(ARG, temperature=1)
    print(sen)
